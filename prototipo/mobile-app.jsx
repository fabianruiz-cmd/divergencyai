  ...
  return (
    <>
      <header className="m-nav">
        <div className="m-nav__spacer" aria-hidden="true"/>
        <button
      ...
      <div className="m-drawer" data-open={drawerOpen}>
        <div className="m-drawer__head">
          <img src={theme === "light" ? "assets/logo-dark.png" : "assets/logo-white.png"} alt="Divergency AI" className="m-nav__logo"/>
          <button
...
function MobileHero() {
  ...
  return (
    <section className="m-hero" id="top" data-theme-mode={theme}>
      ...
      <a href="mobile.html" className="m-hero__logo" aria-label="Inicio Divergency AI">
        <img src={theme === "light" ? "assets/logo-dark.png" : "assets/logo-white.png"} alt="Divergency AI"/>
      </a>
      <div className="m-hero__content">
        ...
        <a href="#contacto" className="m-hero__cta">Te contactamos</a>
...
/* ═══════════════════════════════════════════════════════════════════════
   PORTAFOLIO (filtros + 3 view modes + swipe desktop)
   ═══════════════════════════════════════════════════════════════════════ */
function MobilePortfolio() {
  const products = window.PORTFOLIO || [];
  const categories = window.PORTFOLIO_CATEGORIES || [];
  const [active, setActive] = useState("todos");
  const [view, setView] = useState("carousel"); // carousel | grid | list
  ...
  // Desktop pointer-drag para el carrusel
  useEffect(() => {
    ...
    const onDown = (e) => {
      ...
      el.classList.add("m-carousel--dragging");
    ...
    const onUp = () => { isDown = false; el.classList.remove("m-carousel--dragging"); };
    ...
    <>
      ...
      <section className="m-section">
        <div className="m-section__head">
          ...
          <div className="m-pf-modes" role="tablist" aria-label="Modo de vista">
            <button className="m-pf-mode" data-active={view === "carousel"} onClick={() => setView("carousel")} aria-label="Carrusel">
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><rect x="1" y="3" width="12" height="8" rx="1.4" stroke="currentColor" strokeWidth="1.3"/><path d="M4 1v12M10 1v12" stroke="currentColor" strokeWidth="1.3" opacity=".5"/></svg>
            </button>
            <button className="m-pf-mode" data-active={view === "grid"} onClick={() => setView("grid")} aria-label="Cuadrícula">
            ...
            <button className="m-pf-mode" data-active={view === "list"} onClick={() => setView("list")} aria-label="Lista">
        ...
        {view === "carousel" && (
          <div className="m-carousel" ref={carouselRef}>
            {filtered.map((p, i) => {
              const num = String(i + 1).padStart(3, '0');
              return (
                <a key={p.id} href={`producto.html?id=${p.id}&m=1`} className="m-card" style={{ '--card-color': p.color }}>
                  <div className="m-card__bg"/>
                  {p.image && <div className="m-card__media"><img src={p.image} alt="" loading="lazy"/></div>}
        ...
        {view === "grid" && (
          <div className="m-pf-grid">
            {filtered.map((p) => (
              <a key={p.id} href={`producto.html?id=${p.id}&m=1`} className="m-pf-grid__card" style={{ '--card-color': p.color }}>
                {p.image && <div className="m-pf-grid__media"><img src={p.image} alt="" loading="lazy"/></div>}
                <div className="m-pf-grid__body">
                  <span className="m-pf-grid__cat">{p.tipo}</span>
                  <h3 className="m-pf-grid__name">{p.nombre}</h3>
                </div>
              </a>
            ))}
          </div>
        ...
        {view === "list" && (
          <div className="m-pf-list">
            {filtered.map((p) => (
              <a key={p.id} href={`producto.html?id=${p.id}&m=1`} className="m-pf-list__row" style={{ '--card-color': p.color }}>
                {p.image
                  ? <div className="m-pf-list__thumb"><img src={p.image} alt="" loading="lazy"/></div>
                  : <div className="m-pf-list__thumb m-pf-list__thumb--ph"/>
                }
                <div className="m-pf-list__body">
                  <div className="m-pf-list__meta">
                    <span className="m-pf-list__cat">{p.categoria}</span>
                    <span className="m-pf-list__tipo">{p.tipo}</span>
                  </div>
                  <h3 className="m-pf-list__name">{p.nombre}</h3>
                  <p className="m-pf-list__blurb">{p.tagline || p.blurb}</p>
                </div>
                <svg className="m-pf-list__arrow" width="13" height="10" viewBox="0 0 14 10"><path d="M1 5h12M9 1l4 4-4 4" stroke="currentColor" strokeWidth="1.6" fill="none" strokeLinecap="round" strokeLinejoin="round"/></svg>
              </a>
            ))}
          </div>
        )}
  ...
  return (
    <div className="m-frame">
      ...
      <div className="m-scroll-area">
        <div className="m-snap m-snap--hero">
          <MobileHero />
          <div className="m-snap__hint" aria-hidden="true">
            <span>Desliza para seguir explorando</span>
            <svg width="16" height="16" viewBox="0 0 14 14" fill="none"><path d="M7 2v10M2 7l5 5 5-5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
          </div>
        </div>
        <div className="m-snap m-snap--portfolio"><MobilePortfolio /></div>
        <div className="m-snap m-snap--thoughts"><MobileThoughts /></div>
        <div className="m-snap m-snap--footer"><MobileFooter /></div>
      </div>
    </div>
  );
}
